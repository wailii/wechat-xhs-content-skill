...
- 固定 6 张图，但**图的结构由体裁决定，不是固定的叙事节奏**。读取对应 format 文件里的「配图规则」章节，按该章节的图号和用途生成。
- 每张图都要写：用途、情绪/视觉基调、场景描述（给 Gemini），横图 16:9。
- 场景描述必须包含：体裁对应的必填字段（见各 format 文件配图规则）。
- 拆解体优先使用截图，截图内容直接描述即可，不需要 Gemini 生成；思想实验体禁止出现人物。
